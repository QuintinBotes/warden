...
- **Contract:** exports the **Sentinel** design tokens (the values from the design artifact: status-semantic color roles `PASS/FAIL/FLAKY/BLOCKED/SKIPPED/QUARANTINED`, base/ink/accent, type scale, spacing, radii), the type pairing **self-hosted Fira Code (mono display/data) + Fira Sans (body)** (validated by the ui-ux-pro-max engine for data-dense operations UIs; `font-display: swap`, no CDN), theme definitions (**Signal** near-black high-contrast = **default**, plus **Watch** slate-teal dark + **Day** light), the **Warden portcullis logo** as SVG components, and headless-styled primitives the dashboard consumes (VerdictCard, StatusPill, CoverageMatrix, TestResultRow, TrendTile, ThemeToggle, **ReplayViewer**).
